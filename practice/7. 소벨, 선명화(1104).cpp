...
void AbsImage(int** img, int height, int width)	//영상에 대해 절댓값
{
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			img[y][x] = abs(img[y][x]);
...
void EX1104_2_SOBEL_XY() {	// 실습9, 10  // 클리핑 조심
	int height, width;
	int** img = (int**)ReadImage("lena.png", &height, &width);
...
void EX1104_3_SOBEL() {	// 실습9, 10, 11  // 클리핑 조심
	int height, width;
	int** img = (int**)ReadImage("lena.png", &height, &width);
	int** sobel_x = (int**)IntAlloc2(height, width);
	int** sobel_y = (int**)IntAlloc2(height, width);
...
#define GetMax(x, y) ((x>y) ? x : y)
#define GetMin(x, y) ((x<y) ? x : y)
#define Clipping(x) GetMax(GetMin(x, 255), 0) // GetMax, GetMin 함수로 클리핑함수 구현하기
abs함수 : 절댓값 구하기
클리핑 : 밝기갑의 범위 내로 값을 변환
스트레칭 : 밝기값의 민감도 조절?
히스토그램 : 모여있는 값을 균일하게 넓게 퍼트림 → 선명하게함
평균필터 : 넓게 있는 값을 평균을 내어서 비슷하게 만듬(좁게 모이게함) → 흐리게함
마스킹 : 값들을 평균을 내어서 잡음을 제거함
미분연산자 : 잡음에 약하다. 엣지를 검출한다.
소벨(마스크) : 엣지를 검출 + 잡음에 상대적으로 강인함
그라디언트보다 잡음이 적고, 엣지가 더 명확함
선명화 : 엣지 + 원본 (밝은 부분은 더 밝게, 어두운 부분은 더 어둡게)
(계수의 합이 1임) , 클리핑 필요
*/