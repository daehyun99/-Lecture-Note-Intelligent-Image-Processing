...
void Avg3x3(int** img, int height, int width, int** img_out) {
	...
	for (int y = 0; y < height; y++)
		for (int x = 0; x < width; x++)
		{
			...
			{
				img_out[y][x] = (img[y - 1][x - 1] + img[y - 1][x] + img[y - 1][x + 1] + img[y][x - 1] + img[y][x] + img[y][x + 1] + img[y + 1][x - 1] + img[y + 1][x] + img[y + 1][x + 1]) / 9.0;
...
void EX1014_1()
{
	...
	C_Histogram(img, width, height, C_Hist);


	ImageShow((char*)"입력영상보기", img, height, width);
	DrawHistogram((char*)"누적히스토그램", C_Hist); // 누적히스토그램을 그려주는 함수
}
...
void EX1014_2() // 
{
	...
	ImageShow((char*)"입력영상보기", img, height, width);
	DrawHistogram((char*)"정규화된 누적히스토그램", NC_Hist); // 정규화된 누적히스토그램을 그려주는 함수
}

///// 누적히스토그램 후, 스트레칭(실습 5, 6) ///// 선명하게함
void EX1014_3()
{
	int height, width;
	int** img = (int**)ReadImage((char*)"lena.png", &height, &width);
