	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++)
		{
			...
			else
			{
				...
				img_out[y][x] = img_out[y][x] / 9.0 + 0.5;
			}
		}
	}
}
...
void AvgNxN(int N, int** img, int height, int width, int** img_out)	// AvgNxN_two와 비교해서 끝에 있는 값은 그대로 유지, AvgNxN_two는 끝에 있는 값은 중복해서 더함
{
	int delta = (N - 1) / 2;
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++)
...
void AvgNxN_two(int N, int** img, int height, int width, int** img_out)	//5주차 - 평균필터의 가장자리 처리 : 끝에 있는 픽셀의 밝기값을 가장 가까운 유효한 픽셀의 값으로 변경
{
	int delta = (N - 1) / 2;
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
...
////////////( 10/21 )////////////////
// 에러가 나오면 출력으로 가서 제일 위에 있는 것부터 찾아보기
// 평균필터 : 고주파를 제거한다. 고주파는 해상도 좋은 곳?
...
void EX1021_1()
{
	...
	Avg3x3_two(img, height, width, img_out);
...
void EX1021_2()
{
	...
	ImageShow((char*)"출력영상보기(3x3)", img_out, height, width);

	// 5x5 필터 구현
	AvgNxN(5, img, height, width, img_out);
	ImageShow((char*)"출력영상보기(5x5)", img_out, height, width);

	// 7x7 필터 구현
	AvgNxN(7, img, height, width, img_out);
	ImageShow((char*)"출력영상보기(7x7)", img_out, height, width);

	// 9x9 필터 구현
	AvgNxN(9, img, height, width, img_out);
	ImageShow((char*)"출력영상보기(9x9)", img_out, height, width);
...
void EX1021_3()
{
	...
	// AvgNxN 필터 구현
	AvgNxN(7, img, height, width, img_out);
	ImageShow((char*)"출력영상보기1(NxN)", img_out, height, width);

	// AvgNxN_two 필터 구현 // 가장자리 처리
	AvgNxN_two(7, img, height, width, img_out);
	ImageShow((char*)"출력영상보기2(NxN)", img_out, height, width);
...
void EX1021_4()
{
	...
	//float** mask는 안됨 : 1차원 배열과 1차원 포인터는 같다. 2차원 배열과 2차원 포인터는 다르다.
